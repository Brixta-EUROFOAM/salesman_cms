// src/app/home/sheetsEditor/serverActions/saveProcessReports.ts
'use server';

import { NEXT_PUBLIC_MYCOCOSERVER_URL } from '@/lib/Reusable-constants';
import { verifySession } from '@/lib/auth';

export async function saveProcessReportsAction(records: any[]) {
  ...
  try {
    const response = await fetch(`${backendUrl}/api/excel/process/save`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      throw new Error(result.error || result.details || "Failed to save process & quality reports");
    }
