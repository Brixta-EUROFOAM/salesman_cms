// src/app/home/sheetsEditor/serverActions/saveFinanceReports.ts
'use server';

import { NEXT_PUBLIC_MYCOCOSERVER_URL } from '@/lib/Reusable-constants';
import { verifySession } from '@/lib/auth';

export async function saveFinanceReportsAction(records: any[]) {
  ...
  try {
    const response = await fetch(`${backendUrl}/api/excel/finance/save`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      throw new Error(result.error || result.details || "Failed to save finance reports");
    }
